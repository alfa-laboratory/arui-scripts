...
if (appPackage['arui-scripts']) {
  throw Error(
    'arui-scripts in package.json is not supported. Use aruiScripts instead.'
  );
}
...
module.exports = {
  ...
  baseDockerImage:
    packageSettings.baseDockerImage || 'heymdall/alpine-node-nginx:8.9.1',
  ...
  serverEntry:
    packageSettings.serverEntry ||
    path.resolve(absoluteSrcPath, 'server/index'),
  ...
  clientEntry:
    packageSettings.clientEntry || path.resolve(absoluteSrcPath, 'index'),
  ...
  localDockerfile: fs.existsSync(dockerfileFilePath)
    ? dockerfileFilePath
    : null,

  useTscLoader: packageSettings.useTscLoader || false,
  useServerHMR:
    typeof packageSettings.useServerHMR !== 'undefined'
      ? !!packageSettings.useServerHMR
      : false,
  useYarn: fs.existsSync(yarnLockFilePath),
  clientServerPort: packageSettings.clientServerPort
    ? parseInt(packageSettings.clientServerPort, 10)
    : 8080,
  serverPort: packageSettings.serverPort
    ? parseInt(packageSettings.serverPort, 10)
    : 3000,
