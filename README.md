...
в немного откличающихся настройках для `babel`, поддержки `ts` и возможности работы с серверным кодом.
...
По-умолчанию на клиенте будет подменяться только css. Для правильной работы с react вам надо добавить прмерно такой код:
```jsx harmony
...
- Замену всех импортов css фалов на пустые файлы
...
Команда `arui-scripts archive-build` запускает компиляцию продакшн версии и сборку архива с скомпилированным кодом.
...
Например такое сожержимое `arui-scripts.overrides.js`:
```javascript
